...
export async function ensureRbacStructureAndSeed(tenantId: string): Promise<void> {
  ...
  const permissions: Array<{ key: string; label: string; group: string; description?: string }> = [
    ...
    { key: 'integrations:read', label: 'Ver integrações', group: 'integracoes' },
    { key: 'integrations:write', label: 'Gerir integrações', group: 'integracoes' },
    ...
    { key: 'stock:costs:read', label: 'Ver custos de stock', group: 'stock' },
